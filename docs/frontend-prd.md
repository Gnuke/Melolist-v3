...
| M3 | 로그인 화면 연결 강화, 플레이리스트 목록/상세, ✅즐겨찾기 목록(07-16), ✅검색 기록 화면(07-16 — `/history`, 목록 패턴은 즐겨찾기와 동일·no_match 행·허밍 일치율 배지·소유 삭제), ✅**Bottom Navigation**(07-16 — 홈·즐겨찾기·기록 3탭, 검색 플로우·로그인 제외(몰입 유지), 활성=전경색만(flame 미사용, §10 주 액션 1개 규칙). 프로필 시트의 임시 진입점은 내비로 통합·제거), 미니 플레이어·인라인 재생은 design-guideline("하단 미니 플레이어 고정 지양")에 따라 보류 |
