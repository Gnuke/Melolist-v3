...
## 2026-07-29 — KR1 1차 실측: 지문 29/30(96.7%) 통과, 허밍 8곡 파일럿 6/8(75%) (코드 무변경 — manifest·오디오는 로컬 전용)

클립 준비가 끝나 KR1 첫 실측을 돌렸다. 지문 30곡은 풀셋(후보 목록 그대로, `audio/fingerprint/<장르>/` mp3)이라 1차 실측치로 유효하고, 허밍은 목표 20곡 중 8곡(지인 2인 아이폰 녹음, `audio/humming/` m4a)이라 **파일럿** — 공식 허밍 판정은 20곡을 채운 뒤로 미룬다. 사전 검사로 MD5 전수 대조(중복 없음)·38트랙 manifest 경로 일치를 확인했다.

**실행**: 로컬 백엔드+실 ACR 키(운영 KR2/KR3 이벤트 오염 방지 — 07-23 계획대로). PS로 `.env` 파싱 env 주입 → `gradlew bootRun` → health UP → `node match-rate.mjs`(38곡, 간격 2s, 약 4분). 리포트 `reports/match-rate-2026-07-29T01-50-45.json`(커밋 금지).

| 모드 | raw | 보정 후 | KR1 목표 | 판정 |
|---|---|---|---|---|
| 지문 | 16/30 | **29/30 (96.7%)** | ≥80% | ✅ 통과 |
| 허밍(파일럿 8곡) | 0/8* | **6/8 (75%)** | ≥50% | ✅ 통과 방향 (비공식, n=8) |

\* 허밍은 곡명 미상 클립이라 title을 `unknown-*`로 두고 **topResult 회수 런**으로 돌린 것 — raw 0/8은 설계된 결과.

### 표기 혼재 실증 → acrid 고정 워크플로 가동

- 지문 raw MISS 14건 중 **13건은 top1이 정답 곡**(score 100)인데 표기만 달라 판정이 샌 것: 아티스트 한글 반환("버즈"·"방탄소년단"·"거미"·"조정석" vs manifest 로마자), 괄호 병기("지코 (ZICO)"·"에픽하이 (EPIK HIGH)" — 괄호 제거 시 한글만 남음), 악센트("Céline Dion"), 공백("벚꽃 엔딩"). **07-23 관측했던 ACR 메타 표기 혼재가 실측으로 그대로 재현** — 예정대로 리포트 `resultAcrid`를 눈으로 검증한 뒤 29곡 acrid를 manifest에 고정했다. judge는 acrid 불일치 시 title/artist로 fallback하므로(match-rate.mjs) 클래식 연주자 변동에도 안전한 순수 추가 안전장치다.
- 허밍도 같은 이유로 acrid 필수 확인: "선물" 반환 표기 "멜로망스 (MeloMance)"는 괄호 제거 시 로마자가 소거돼 artist 매칭이 새고, 커버 버전 반환(아래) 시 artist가 아예 다르다.

### 진성 미스 3건 — 성격이 전부 다르다

1. **지문 '엘리제를 위하여'**: "I Gotta Feeling — Dean Millen"이 **score 100**으로 반환 — 지문 정확 매칭인데 오답, ACR DB 오라벨 의심. 클립 확인·재녹음 검토 대상(acrid 미기입).
2. **허밍 '거리에서'**: 정답은 성시경 곡인데 **윤종신 동명이곡이 0.96으로 반환** — 동명이곡/커버로 새는 오매칭 패턴의 첫 표본. 20곡 채우면 이 패턴을 따로 셀 것.
3. **허밍 '좋은 날'(IU)**: **무결과** — 국민가요급이 무결과인 건 이례적이라 클립 음량·구간 확인 여지(곡 탓 vs 클립 탓 분리 필요).

### 허밍 관찰 (n=8이라 전부 잠정)

- **커버 버전으로 반환되는 특성**: Let It Be→Vázquez Sounds, 모든 날 모든 순간→이찬원, 양화대교→inst. — 곡 식별은 정확하므로 HIT 취급, acrid는 해당 커버 기준(다른 커버 반환 시 MISS 가능성 메모).
- **글로벌 3/3 vs K-곡 3/5** — "ACR 허밍 DB의 K-pop 커버리지 부족" 가설(07-16 오매칭 진단)과 방향 일치. 확정은 20곡 공식 런에서.
- 곡명 미상 클립 워크플로가 유효했다: `unknown-*` placeholder 런 → topResult 힌트 → **사용자가 클립을 듣고 귀로 검증·확정**(검색 결과를 검증 없이 정답 삼지 않기) → title/artist+acrid 기입. 8곡 전곡 이 경로로 확정.
- 허밍 클립에 `singer`/`device` 메모 필드 유지(스크립트가 무시하고 리포트에 실어줌) — 녹음자·기기별 미스 편중 분석용. 추가 12곡도 동일 방식.

### 운영 메모

- **ACR 콘솔 사용량 통계는 집계 지연 수 시간** — 실측 직후 대시보드 0건이라 놀랐으나 실호출은 응답 내용(실 acrid·score)으로 자명했고, 수 시간 뒤 반영 확인. 지문/허밍은 별도 프로젝트라 각각 찍힌다.

**남은 것**: ①엘리제 지문 클립 확인(필요 시 재녹음) ②좋은 날 허밍 클립 상태 확인 ③허밍 12곡 추가 녹음 ④20곡 공식 KR1 런 + 체크인(분기 3회 측정의 1회차는 이번 지문 수치로 확보).

## 2026-07-24 (2) — AI 폴백 과잉 기권 수정: "뭘 입력해도 empty" 진단→프롬프트 조정+원응답 로깅 (✅**PR #36 병합** `fe97d51`, 운영 스모크 통과)

"어제 자연어 보충 검색에 뭘 입력해도 결과가 안 나오던데 목데이터만 적용된 기능이었나?" 제보에서 출발 — event_log 확인 결과 07-23 운영 실쿼리 3건 전부 `outcome=empty`, 단 `ai_ms` 4~7.6s로 **실 OpenAI 호출은 정상 수행 중**(목 아님, mock은 일정하게 ~0.8s). 로컬 실키 재현으로 원인을 판정하고 같은 날 수정·배포까지 종결했다.

### 진단 — 스키마가 아니라 프롬프트의 과잉 기권

- 가설 2개: ①모델의 정상 기권(프롬프트 규칙 "확실한 후보가 없으면 빈 목록") ②조용한 스키마 미스매치(`entity()` 바인딩이 `candidates=null`로 떨어져도 겉으론 empty — 기권과 구분 불가). 기존 계측은 query_len만 남겨 소급 판별 불가 → **후보 0건 시 LLM 원응답을 로그로 남기는 진단 로깅을 먼저 넣고** 재현 스모크.
- 판정: 기권 시 원응답이 명시적 `{"candidates":[]}` — 스키마 정상, **규칙의 "확실한 후보"가 매칭 확신으로 해석돼 모호한 설명에 전부 기권**하는 것이 원인. 정답 유형(벚꽃엔딩·Ditto·Rolling in the Deep)은 여전히 1순위로 나와 파이프라인 자체는 건강함도 확인.

### 수정 (`OpenAiSongFinderClient`, 커밋 `bce55d8`)

- **① 프롬프트 기권 기준 재조정**: 실존 확신(후보 단위 — 지어내기 금지, 확신 없는 조합은 제외)과 매칭 확신(순위로 표현)을 분리. 단서와 맞는 실존 곡이 전혀 없을 때만 기권.
- **② 진단 로깅**: `entity()` → 수동 변환(`BeanOutputConverter`, 동일 스키마·포맷 지시)으로 전환 — 후보 0건·파싱 실패 시 원응답을 서버 로그에 기록(2000자 컷). 파싱 실패는 기존대로 `outcome=error`(502)로 empty와 구분. 다음 empty 재발 시 Render 로그만으로 기권/스키마 판별 가능.

### 검증 — 실키 스모크 7종 × 3라운드 (정답 3 + 모호 3 + 무의미 대조군 1)

- 정답 유형 3건 **1순위 유지** · 모호 설명 3건(8·33·79자 유형) **실존 후보 5곡씩** · 무의미 대조군("고장난 선풍기 트로트") **기권 유지(0건)**.
- **중간 함정(2라운드)**: 기권 완화만 적용하니 대조군에 실존 의심 후보(조용필 "고장난 타이어" 등 환각)가 섞임 → **후보 단위 실존 조건 강화(규칙 ①)로 해소** — R5(환각 방지) 유지가 완화의 전제 조건임을 확인.
- backend build·테스트 83건 GREEN. 스모크 이벤트 21건(로컬)+2건(운영) event_log 정리.
- **운영 스모크(배포 직후)**: 모호 쿼리→hit 5곡, 무의미→empty 2.2s. 웜업 직후 첫 AI 호출 `ai_ms` 10,002ms(10s 컷 스침)·total 14s — 기존 "웜업 직후 첫 AI 검색 타임아웃 가능(F4 재시도 커버)" 주의 재확인. gpt-5.4-mini 지연 변동 2.4~10.1s 관측.

### 재사용 — 로컬 실키 스모크 절차

backend에서 PS로 `.env` 파싱해 env 주입 + `AI_QUOTA_GUEST_DAILY` 상향 + `SPRING_PROFILES_ACTIVE=''` + bootRun → Node 스크립트로 `POST /api/search/text`(한글 curl 인코딩 회피) → 테스트 session_id 이벤트 삭제. **로컬 백엔드도 운영 Supabase에 붙으므로 이벤트 정리 필수.**

**남은 것**: 변동 없음 — KR1 클립 50개 녹음이 유일한 큰 건.

## 2026-07-24 — 운영 첫 접속 진단 2건: 재생 무음(기기 라우팅 추정)·검색 F4(콜드스타트 실증) (코드 무변경)

"첫 접속에서 녹음했는데 안 들리는 경우가 있다 — 콜드스타트 영향인가?" 제보에서 출발, 증상을 둘로 분리해 각각 판정했다.

- **허밍 확인 카드 재생 무음(일시적)**: 파형은 그려지고 진행 커서도 움직이는데 소리만 안 들림. 판정 — 재생 소스는 로컬 blob(`URL.createObjectURL`)이라 **네트워크·콜드스타트는 원인 불가**, WaveSurfer 파형은 blob 실디코드 결과라 **녹음 자체는 정상**. 유력 원인은 **getUserMedia 캡처 세션 직후 오디오 세션이 통화 모드에 남아 재생이 수화부/통화 볼륨 채널로 라우팅**되는 알려진 모바일 브라우저 현상(웹 코드로 제어 불가, 모바일은 setSinkId 미지원) → **수정 대상 아님, 재현 빈도 관찰**. 재현 시 확인 3항목: ①재생 중 볼륨 버튼의 볼륨 종류(미디어 vs 통화) ②수화부에서 들리는지 ③블루투스 연결.
- **첫 접속 검색 실패(F4)는 콜드스타트 실증**: 07-23 세션 event_log에서 `visit` 직후 허밍 `timeout` 6건이 수 초 안에 버스트로 적재 — 계측 fetch(keepalive)가 잠든 서버 기동까지 걸려 있다가 일괄 인서트된 것. 서버 기동 직후 첫 `search_request` 성공(total 8.2s), 이후 3~5s 정상. 15s 클라 타임아웃 반복 → F4 → 서버 기동 후 재시도 성공이라는 콜드스타트 전형 패턴.
- **계측 공백 메모**: `search_failed(reason=bad_audio)`에 silent/short variant·meanLevel·durationMs 미포함 — 무음 이슈가 재발하면 데이터로 못 가르므로 프론트 계측 보강 후보.

"AI 화면 개발은 실측 이후에 하는 게 의미가 있나?" 질문에서 출발 — 현황을 코드·계약으로 점검한 뒤 **화면 개발은 전부 나중으로, 데이터(계측)만 있으면 됨(주 목적=포트폴리오)**으로 확정했다. 실측(KR1)이 화면의 근거·재료를 만들어주는 구조라 순서상으로도 실측 선행이 맞다.

- **폴백 화면 현황 확인**: `FallbackSearchView`는 spec 002(PR #19)로 이미 운영 배포됨 — 별도 라우트가 아니라 SearchPage의 phase. 진입점 2곳: ①F2 무결과 실패 화면 "말로 설명해서 찾기"(허밍은 주 버튼 — ACR 허밍 커버리지 한계의 구제 경로) ②오매칭 결과 화면 하단 버튼(원 결과 보관→"뒤로" 복귀). **없는 것은 화면이 아니라 독립 진입점(홈·탭)** — 승격 여부가 곧 "AI 화면 개발"의 실체이고, 그 근거는 KR1 허밍 실측치.
- **계측 점검 — 허밍 실패→폴백 종결 전 구간이 이미 event_log 적재**: `search_request`(no_match 포함, 서버) → `ai_fallback_open`(from=no_match|mismatch, 프론트) → `ai_search_request`(**outcome=hit|empty|error|quota** + ai_ms·total_ms·candidates, 서버) → `ai_search_select`(rank·resolved) → `ai_search_cancel`. 서버 계측은 recordSilently(REQUIRES_NEW)라 본 흐름 실패에도 기록 유지.
- **어드민 노출 범위**: 허밍 실패까지는 대시보드에 보임(failures mode×reason·KR2 matched_n). **AI 폴백 지표는 admin metrics 계약에 없음**(spec 002·003 병렬 개발 탓) — 산식 정본은 `kr_metrics.sql` SC-001~004 절(폴백 시도율·채택률·응답 p95·전환율)이라 당분간 SQL 직접 실행으로 조회.
- **보류 확정 2건**: ①폴백 독립 진입점 승격(KR1 실측 결과가 투자 근거) ②어드민 metrics AI 블록(추가 시 admin-api.md 계약+백+어드민 프론트 같은 PR — 원칙 II). 포트폴리오 서사도 "실측→데이터로 판단→화면"이 강하다.
- **로컬 데모 방법 확립(재사용 가능)**: 폴백 화면은 **프론트 목업 단독**으로 재현 — `searchMock.ts`에서 `MOCK_SEARCH_ENABLED=true`+`MOCK_SCENARIO='nomatch'`(F2 유도)+`MOCK_TEXT_SCENARIO='hit'`, 백엔드 불요·dev 전용. AI 폴백 mock은 empty/quota/error/slow 시나리오까지 커버. 확인 후 스위치 원복(커밋 금지 관례). **함정: 목업 세션은 track이 fire-and-forget이라 event_log에 아무것도 안 남음** — 계측 확인은 실백엔드에서만.

**남은 것**: 변동 없음 — KR1 클립 50개 녹음(사람 손)이 유일한 큰 건. 실사용 데이터 쌓이면 SC-001~004 첫 산출.

## 2026-07-23 (5) — Spring AI 2.0 마이그레이션 사전 조사 + 현행 스택 정량 기준선 측정 (✅**PR #34 병합** `ae09444`, 마이그레이션 실행은 별건으로)

"Spring AI 2버전 LTS 나오지 않았나?" 질문에서 출발 — 조사 결과 **Spring AI 2.0.0 GA는 2026-06-12 릴리스**(Boot 4.0·Framework 7.0 필수), 그리고 현행 스택(Boot 3.5·Spring AI 1.x)은 **2026-06-30부로 OSS 지원 종료**(EOL 경과 — 보안 패치 없음, HeroDevs가 Spring AI 관련 CVE-2026-22729/22730 언급 → 해당 여부 별도 확인 필요). 토큰 비용은 OpenAI 모델 단가·토큰량으로 결정되므로 Spring AI 버전과 무관(오해 정정). 즉 마이그레이션 사유는 "비용"이 아니라 "EOL" → 실행 전에 **전후 정량 비교(포트폴리오 소재)를 위한 현행 기준선**을 먼저 측정해 기록한다.

**측정 환경**: Windows 10 Pro(10.0.19045) · Temurin 21.0.11+10-LTS · Gradle 8.14.5(데몬 워밍업 후) · main `02a0144` · 기동은 .env 실구성(Supabase 원격 pooler DB, ap-northeast-2)

| 지표 | 현행(Boot 3.5.16 + Spring AI 1.1.8) | 측정 방법 |
|---|---|---|
| clean assemble | **21s** (2회 동일) | `gradlew clean assemble` — 데몬 워밍업 후 2회 |
| test 태스크 | **51s** (테스트 컴파일 포함) / 스위트 합산 **5.8s** | `gradlew test` 83건 GREEN · `build/test-results` XML 합산(16 suites) |
| bootJar 크기 | **68.86MB** (72,209,154 bytes) | `build/libs/backend-0.0.1-SNAPSHOT.jar` |
| 기동 시간 | **34.5s / 38.7s / 42.7s** (3회) | `java -jar` 로그 "Started BackendApplication in" — 원격 DB 왕복(Hikari 초기화+JPA validate) 지배라 변동폭 큼 |
| 힙(기동 직후 Full GC 후) | used **43.7MB** / committed 148MB | `jcmd <pid> GC.run` → `GC.heap_info` |
| Metaspace | used **90.0MB** (class space 12.7MB) | 〃 |
| RSS | **327.5MB** (Private 393.2MB) | 프로세스 WorkingSet64 |

**핵심 의존성 해석 버전 스냅숏**(마이그레이션 시 전부 메이저 점프 예상 — 전후 대조용): Boot **3.5.16** · Framework(spring-core) **6.2.19** · Security **6.5.11** · Hibernate ORM **6.6.53.Final** · Tomcat embed **10.1.55** · Jackson **2.21.4** · Spring AI **1.1.8** · postgresql 드라이버 42.7.11

**2.0 주요 변경(사전 조사)**: Boot 4.0+Framework 7.0 필수(=Boot 마이그레이션 선행) · OpenAI 모듈 3종(Azure/HTTP/SDK)→SDK 1종 단일화 · 옵션 immutable 빌더화 · 프로퍼티 키에서 `.options` 세그먼트 제거(`spring.ai.openai.chat.options.model` 경로 변경 → application.yml 수정 필요) · 툴 콜링 루프의 advisor 체인 이동 · Jackson 2→3. 우리 사용면은 ChatClient 단일 호출(spec 002)이라 Spring AI측 수정량은 작고, 실질 작업량은 Boot 3.5→4.0 쪽.

**측정 제외(사유 명시)**: AI 폴백 응답시간(OpenAI API 왕복이 지배라 프레임워크 차이 검출 불가) · Render 콜드스타트(무료 티어 변동 지배). 기동 시간도 원격 DB 왕복 포함이라 **전후 비교는 동일 머신·동일 네트워크·동일 방법 3회 재측정**으로 대조할 것.

**남은 것**: Boot 4 + Spring AI 2.0 마이그레이션 실행(별건, 착수 시점은 미정) → 본 표와 동일 방법 재측정 → 차이를 포트폴리오에 기록.

## 2026-07-23 (4) — 백엔드 서비스 단위 테스트 백필: Playlist·Review·AiQuota 20건 (✅**PR #32 병합** `5facc02`, 브랜치 `test/backend-service-backfill`)

"백엔드 TDD는 안 만들어도 되나?" 질문에서 출발 — 점검 결과 백엔드 테스트는 63건 존재했고, 서비스 7곳이 공백이었다. **로직 분기가 있는 3곳만 선별 백필**하고 얇은 계층(EventService·AdminAuditService·RecommendationProvider 등 단순 위임/적재)은 비용 대비 이득이 없어 의도적으로 제외. 프로덕션 코드 무변경(테스트 파일 3개 추가만).

- **PlaylistServiceTest (10건)**: 소유권 분기 — 비공개는 비소유자에게 **404로 존재 숨김**, 공개라도 변경 시도는 403(부록 A-2 앱 계층 단독 검증) · 게스트의 공개 상세 조회 · 담기 맨 뒤 position 부여와 중복 409 · 트랙 삭제 후 position 0..n-1 압축 · reorder 순서 재매김과 목록 불일치 400 두 갈래(개수 불일치·미포함 id).
- **ReviewServiceTest (6건)**: 1인 1건 409 유도 · **동시 요청이 unique(user_id) 제약에 걸려도 409로 수렴**(DataIntegrityViolationException 캐치 경로) · 작성자 아닌 수정 403 · PATCH null 필드 유지 · getMine 404 · 첫 리뷰 저장.
- **AiQuotaServiceTest (4건)**: 게스트=세션 3회 / 로그인=계정 10회 분기(spec 002 R6 — 각자 반대쪽 카운트 경로를 타지 않는 것까지 verify) · 카운트 기준 시각이 **Asia/Seoul 오늘 자정**인지 ArgumentCaptor로 검증 · 429 예외의 limit·reset_at(다음날 자정) 계약. AiProperties는 mock 대신 **실제 record 인스턴스 주입**(@InjectMocks 불가라 수동 생성 — mock 아닌 실 데이터 구조가 더 정직한 테스트).
- **검증**: `gradlew test` 전체 스위트 **83건(기존 63 + 신규 20) BUILD SUCCESSFUL**.
- **함정(Mockito)**: `thenReturn(...)` 인자 안에서 내부적으로 스터빙하는 헬퍼(`track()`이 `lenient().when(music.getId())` 수행)를 호출하면 바깥 스터빙이 미완료 상태라 `UnfinishedStubbingException` — 헬퍼 호출을 `when()` 앞 문장으로 하이스트해 해결. 2건이 이걸로 첫 실행 RED였다.

**남은 것**: 변동 없음 — KR1 첫 실측(클립 50개 녹음 대기) · KR2 재확인(실사용 데이터 대기).

## 2026-07-23 (3) — 프론트 테스트 기반 도입(vitest 35건) + README 3종 현행화 (✅**PR #29·#30·#31 병합**)

테스트·문서의 묵은 공백 두 갈래를 같은 날 해소했다. 백엔드는 단위 테스트 13파일을 쌓아 왔는데 **프론트는 0개·러너조차 미설치**였던 것(#29), README 3종이 07-20 시점에 멈춰 **AI 폴백(spec 002)·어드민(spec 003)이 통째로 누락**돼 있던 것(#30·#31) — 둘 다 사용자 질문으로 표면화됐다.

| PR | 내용 | 갈래 |
|---|---|---|
| #29 | **프론트 단위 테스트 도입** — vitest 4.1 + jsdom + @testing-library/react(react 19 호환 v16). `vitest.config.ts`는 **별도 파일**(vite.config의 tailwind 플러그인을 테스트에 안 끌고 오기 위해 — alias `@`만 복제). 테스트 35건 5파일, 전부 순수 로직·특성화: `resultStash` 11(TTL 경계 fakeTimers·StrictMode peek 비파괴) · `recentFinds` 10(dedup·MAX6·mock 필터) · `theme` 5 · `session` 3 · `useMe` 6(**계정 전환 가드** — api 모듈만 vi.mock, pending Promise로 placeholder만 노출) | 테스트 |
| #30 | **README 3종 현행화** — 루트: 주요 기능 3건(AI 폴백·어드민·테마)+스택 AI 행+구조 admin · backend: 도메인 트리를 실제 패키지와 일치화(event·admin 추가, recommendation=인터페이스 슬롯으로 정정, "Spring AI(M5) 대비" 문구 제거) · frontend: 폴더 트리 현행화(features 7종·pages/admin·mock·lib/theme). vitest 표기는 **의도적 제외**(#29 병합 선행 합의) | docs |
| #31 | README 프론트 테스트(vitest) 표기 — #30에서 미뤄 둔 것의 #29 병합 후속 반영 | docs |

**핵심**:

- **뮤테이션 검증 수행(#29)**: useMe의 계정 전환 id 가드를 일부러 제거하니 **정확히 가드 테스트 2건만 RED** — 복원 후 green. 특성화 테스트의 실효성을 증명하고 커밋.
- 검증: 35/35 green + `npm run build`(tsc가 테스트 파일도 타입체크) + oxlint(기존 fast-refresh 경고 4건만).
- 함정: jsdom(vitest 4 동봉)은 localStorage·sessionStorage·crypto.randomUUID를 다 갖춰 폴리필 불필요 · react-query v5의 `placeholderData`는 **disabled 쿼리에도 적용됨**(계정 가드 테스트에 그대로 활용).

## 2026-07-23 (2) — KR1 실측 준비 완료: 테스트 곡 셋 확정 + manifest 작성 + 리포트 acrid 보강 (✅**PR #27 병합**)

"곡 셋 오디오 대기"로 멈춰 있던 KR1을 녹음만 남은 상태까지 끌어올렸다. 시작은 "admin에 KR1 화면 없지?" 확인 — 없는 게 맞고 **설계상 의도**(KR1은 운영 event_log 산출이 아니라 테스트 곡 셋+정답 기대값을 수동 스크립트로 재는 지표라 어드민 metrics API로 계산 불가. 어드민 노출은 실측 시작 후 측정 결과 적재 테이블로 고려).

- **곡 셋 후보 확정**(`backend/scripts/match-rate/song-set-candidates.md` 신규): 지문 30(K-pop 10·팝 7·OST 5·힙합 4·클래식 4, 1975~2024 연도 분산) + 허밍 20(원곡 보컬 남/녀 배분) + 교체 스페어 5. **허밍 셋에 글로벌 스탠더드(Let It Be·캐논·My Heart Will Go On) 의도 포함** — 글로벌 곡은 붙고 K-발라드만 빠지는 패턴이 나오면 오매칭 진단의 "ACR 허밍 DB K-pop 커버리지 부족" 가설이 실측으로 확정된다.
- **녹음 기준 문서화**: 지문=유튜브 공식 음원 재생→폰 마이크 10초 내외(폰 m4a는 스크립트 MIME 맵 지원 확인, 변환 불필요) · 허밍=**10~15초 후렴**(하한 README 8초, 상한 앱 실사용 20초 자동 정지) · **창법 자유**(허밍/라라라/가사 창법 — ACR 허밍 인식은 가사가 아닌 음정 윤곽 매칭이라 전부 유효, 가사 창법이 음정이 정확해져 오히려 유리) · 50곡 길이·환경 동일 조건 통일.
- **핵심 발견 — ACR 메타 표기 혼재**: 운영 music 테이블 관측 결과 같은 한국 곡도 제목이 한글("귀로")/영문("Last Love") 혼재, 아티스트는 로마자 우세("NAUL"·"Lee Sun Hee"·"K.Will"). 판정이 title+artist 느슨 일치라 표기가 어긋나면 HIT가 MISS로 새는 측정 오차 → manifest 표기 규칙(한국어 곡=한글 제목+로마자 아티스트, 클래식=artist 생략+느슨 제목) + **1차 실행 후 acrid로 판정 고정** 워크플로 채택.
- **`match-rate.mjs` 보강**: 리포트 행에 `resultAcrid`(HIT=매칭 결과·MISS=top1) 추가 — 기존 리포트엔 결과 acrid가 없어 "리포트에서 acrid 회수" 워크플로가 실제로는 불성립이었다. 이 두 줄로 성립.
- **`manifest.json` 50트랙 로컬 작성**(gitignore 로컬 전용 — 커밋 미포함): 파일명 규칙(`<슬러그>-clip`/`-humming`.m4a), 파싱·모드별 카운트(30/20) 검증. 상단 `_표기규칙`·`_클래식주의` 키에 규칙 명시.

**남은 것**: 클립 50개 녹음(사람 손) → 1차 실측은 **로컬 백엔드+실 ACR 키** 경유(운영 KR2/KR3 이벤트 오염 방지, README 권고) → MISS 중 표기 차이분 resultAcrid 검증·manifest 고정 → KR1 첫 수치·주간 체크인 기록.

## 2026-07-23 총괄 — PR 5건(#21~#25) 전부 병합·운영 확인 종결: 문서 트리 확정 + 탭 셸 UX 일제 정비(헤더·게스트·프로필 캐시·테마)

기능 마일스톤 사이의 다듬기 날 — 사용자 실사용 제보(헤더 흔들림 → 게스트 탭 이탈 → 재로그인 프로필 공백)를 따라가며 탭 셸 UX를 한 바퀴 정리했다. 5건 모두 당일 병합·운영 확인까지 종결. 이날부터 프로세스 변경: **PR 종결(병합·클로즈)은 사용자가 직접**, Claude는 PR 생성까지(명시 요청 시에만 병합). 종료 시점 리포 main 단일(`0f08a76`), 열린 PR 0.

| PR | 내용 | 갈래 |
|---|---|---|
| #21 | **docs 트리 재구성**(07-22 작성분 병합) — docs/를 prd·guides·devlog(월별)·archive 4폴더로, OKR 승격(`docs/prd/okr-2026q3.md`), 기준 정본=docs/README.md 색인 | docs |
| #22 | 🐛 **탭 전환 헤더 흔들림** — 원인 3중: ①홈만 pt-6+워드마크(≈31.5px), 나머지는 pt-4+h-8(32px)라 우측 아바타가 탭 간 **~8px 수직 점프**(게스트는 아바타가 없어 미체감 — "로그인만 흔들림" 제보와 정확히 일치) ②홈 헤더 motion 진입(y+14)이 홈 복귀마다 재생 ③목록 있는 탭만 스크롤바(10px)가 생겨 중앙 컨테이너 좌우 ~5px 시프트 → 탭·드릴인 6화면 헤더 **pt-4+h-8 공통 규격** + 홈 헤더 정적화(히어로 이하 스태거는 유지) + `scrollbar-gutter: stable` | 결함 |
| #23 | **게스트 보호 탭 인라인 로그인 유도** — 즐겨찾기·기록·플레이리스트의 `Navigate(/login)` 가드(의도된 설계였으나 탭만 눌러도 셸 이탈)를 **탭 셸 유지 + 공용 `GuestPrompt`**(기존 빈 상태 패턴, 탭별 brand/iris 톤)로 교체. /login 이동은 CTA를 눌렀을 때만, `state.next` 복귀(FR-004)는 그대로. LoginPage 뒤로가기도 항상 홈 → next 화면 복귀로. 부수: 게스트 부제가 disabled 쿼리의 isPending=true 탓에 "불러오는 중…" 오표기되던 것 수정 | UX |
| #24 | 🐛 **재로그인 프로필 공백** — 로그아웃이 localStorage 프로필 사본까지 지워(PR #14 placeholder 무력화) 재로그인 첫 렌더가 /users/me 왕복 동안 비어 보임 → `clearCachedMe()` 제거(사본 유지, 메모리 쿼리만 삭제). 계정 전환 노출 방지는 `readCachedMe`의 id 가드(사본 id==현재 uid)가 이미 보장하던 **이중 안전장치**였음 — 이제 유일 가드(주석 명시) | 결함 |
| #25 | **테마 토글 헤더 승격**(07-22 "남은 것"의 게스트 토글 진입점 실현, 사용자 제안) — 공용 `ThemeToggle`(ghost icon-sm, 아이콘=누르면 바뀔 모드)을 탭 4화면 헤더 우측 [토글, ProfileCorner]로 배치, **게스트도 테마 전환 가능**. 프로필 "화면 테마" 세그먼트 섹션 제거(이동). 저장 로직(theme.ts·index.html 부트스트랩·localStorage 키) 무변경 | UX |

**핵심 발견·진단**:

- **#22·#23·#25는 한 줄기** — 게스트가 탭 셸에 머무는 시간이 길어지면서(탭 구경 → 인라인 유도 → 테마 전환까지) "게스트도 1급 사용자" 방향이 셸 전반에 반영됐다. 탭 헤더는 이제 전 화면 공통 규격: pt-4 + h-8, 우측 [테마 토글, ProfileCorner].
- **#24 진단 순서가 유효했음**: 서버 전 구간 정상을 먼저 실증(Supabase auth 로그 재로그인 4회 성공 · Render 웜 0.5s 응답 · profiles 행 온전 · getOrProvision은 기존 행 무덮어쓰기)한 뒤 프론트로 좁힘 — "잠깐 비었다 채워짐" 사용자 확인으로 일시 공백을 확정하니 원인은 로그아웃의 과잉 캐시 삭제 한 줄이었다.
- **"기본이 라이트로 보임" 문의(#25 확인 중)**: 기본 다크의 정본은 `index.html:2` `<html class="dark">` + 부트스트랩(저장값 'light'일 때만 해제, OS 설정 미참조) — 라이트로 뜬 건 localStorage 잔존 선택값이 원인. **localStorage는 origin별**(localhost:5173 ↔ vercel.app 별도 저장)이라 로컬 테스트 때 헷갈리기 쉽다. 시크릿 창 다크 확인으로 종결.
- **트레이드오프 수용(#24)**: 로그아웃 후에도 마지막 계정의 표시 정보(이름·이메일·아바타 URL)가 그 브라우저 localStorage에 남음 — 본인 기기 위주 서비스라 수용(사용자 결정).

**남은 것**: KR1 첫 실측(곡 셋 오디오 대기) · KR2 재확인(실사용 데이터). 후속 후보: OS `prefers-color-scheme` 추종(저장값 없을 때의 초기값), 어드민 후속 화면(곡 상세/삭제·사용자 상세·모더레이션).
