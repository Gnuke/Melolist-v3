...
- [ ] T030 실키 스모크(quickstart §6) — OPENAI_API_KEY로 실호출 3~5건 품질·ai_ms 확인, OpenAI 단가 재확인(R1 비고), Render 환경변수(OPENAI_API_KEY·AI_MODEL) 등록 및 웜업 후 확인 *(⏳ 사용자 키 필요 — backend/.env의 주석 해제 후 수행)*
- [ ] T031 병합 게이트 최종 확인 — backend 빌드·validate·테스트, frontend build, quickstart 전 시나리오, 시크릿 커밋 미포함(원칙 IV) 점검, 폴백 요청 페이로드에 오디오 미포함 확인(FR-012) *(⏳ 07-21: 실키 스모크(T030) 제외 전 항목 확인 완료 — 빌드·테스트 33건·mock E2E 18건·시크릿 미추적·텍스트 전용 페이로드 확인)*
